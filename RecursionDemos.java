...
//		printMP(0, 0, 2, 2, "");
		...
		ArrayList<String> list = getKPC("12");
		for (String str : list)
			System.out.println(str);

		System.out.println(list.size());
		System.out.println(getKpcCount("1312111113"));
	}

	public static int getKpcCount(String str) {
		if (str.length() == 1) {
			return getChoice(str.charAt(0)).length();
		}

		char ch = str.charAt(0);
		String ros = str.substring(1);
		int count = getKpcCount(ros);
		return getChoice(ch).length() * count;
	}
